 ...
 * (as defined by both the ANSI SQL and JPA specs) in cases where the dialect may not support the full {@code trim}
 * function itself.
 * <p>
...
public abstract class AbstractAnsiTrimEmulationFunction implements SQLFunction {
	 ...
	 * <p>
	 * In this form, the imput arguments is missing the {@code FROM} keyword.
	 *
	 ...
	 * Resolve the function definition which should be used to trim both leading and trailing spaces.
	 * <p>
	 * The same as {#link resolveBothSpaceTrimFunction} except that here the{@code FROM} is included and
